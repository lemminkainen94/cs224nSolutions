...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            prev = [sentence[j] if j >= 0 else start for j in range(i - window_size, i)]
            next = [sentence[j] if j < len(sentence) else end for j in range(i + 1, i + window_size + 1)]

            window = [feature for features in prev for feature in features] + sentence[i] \
                        + [feature for features in next for feature in features]
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier((Config.n_window_features * Config.embed_size, Config.hidden_size)))
        b1 = tf.zeros(Config.hidden_size)
        U = tf.Variable(xavier((Config.hidden_size, Config.n_classes)))
        b2 = tf.zeros(Config.n_classes)
    ...
    def add_training_op(self, loss):
        ...
        op = tf.train.AdamOptimizer(Config.lr)
