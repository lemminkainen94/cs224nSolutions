...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length:
            padded_sentence = sentence + [zero_vector for i in xrange(max_length - len(sentence))]
            padded_labels = labels + [zero_label for i in xrange(max_length - len(labels))]
        else:
            padded_sentence = sentence[:max_length]
            padded_labels = labels[:max_length]

        mask = [True] * min(max_length, len(sentence)) + [False] * (max_length - len(sentence))
...
class RNNModel(NERModel):
    ...
    def add_training_op(self, loss):
        ...
        op = tf.train.AdamOptimizer(learning_rate=Config.lr)
